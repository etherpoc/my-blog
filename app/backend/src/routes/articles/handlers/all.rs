...
pub async fn all(
    ...
    let articles = match &query_params.tag {
        Some(tag) => Article::get_all_by_tag(pool.get_ref().clone(), true, tag.clone(), 20, page, order_option).await,
        None => Article::get_all(pool.get_ref().clone(), true, 20, page, order_option).await,
    };
