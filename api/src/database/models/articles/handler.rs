...
impl Article {
    ...
    pub async fn insert(pool: sqlx::PgPool, title: String, img_url: Option<String>, tags: Vec<String>, content: String) -> Result<Article, sqlx::Error>{
    ...
    pub async fn update(pool: sqlx::PgPool, id:i32, title: String, img_url: Option<String>, tags:Vec<String>, content:Option<String>, visibility: bool) -> Result<Article, sqlx::Error>{
        let tags_for_query = tags_to_string(tags);
        let img_url_for_query = match img_url {
            None => "NULL".to_string(),
            Some(x) => x
    ...
    pub async fn _increment_pv(pool: sqlx::PgPool, id:i32) -> Result<Article, sqlx::Error>{
        let sql = format!(
            "UPDATE articles SET \"pv\" = \"pv\" + 1 WHERE id={}", id);
        sqlx::query_as::<_, Article>(sql.as_str()).fetch_one(&pool).await
    }
      
    pub async fn delete(pool: sqlx::PgPool, id: i32) -> Result<Article, sqlx::Error>{
        let sql = format!(
